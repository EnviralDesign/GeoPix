...
f = open("%s\\Supporter_Patrons.txt"%(basePath), "w", encoding="utf-8")
f.write( '\n'.join(supporter_tier) )
f.close()

f = open("%s\\Official_Patrons.txt"%(basePath), "w", encoding="utf-8")
f.write( '\n'.join(official_patron_tier) )
f.close()

f = open("%s\\Super_Patrons.txt"%(basePath), "w", encoding="utf-8")
f.write( '\n'.join(super_patron_tier) )
f.close()

f = open("%s\\Vip_Patrons.txt"%(basePath), "w", encoding="utf-8")
f.write( '\n'.join(vip_patron_tier) )
f.close()

f = open("%s\\Bronze_Patrons.txt"%(basePath), "w", encoding="utf-8")
f.write( '\n'.join(bronze_sponsor_tier) )
f.close()

f = open("%s\\Silver_Patrons.txt"%(basePath), "w", encoding="utf-8")
